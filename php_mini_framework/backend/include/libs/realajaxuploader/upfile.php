...
$upload_path = _assets.$upload_path;
...
if(isset($_FILES['ax_file_input'])) 
{
	...
	{
		$response = array('name'=>$file_name, 'size'=>$_FILES['ax_file_input']['size'], 'status'=>-1,
...
if($currByte==0)
{
	...
	{
		$response = array('name'=>$file_name, 'size'=>$full_size, 'status'=>-1, 'info'=>'File size exceeded maximum allowed: '.$max_file_size);
		return false;
	}
	...
	if(!$file_name)
	{
		$response =array('name'=>$tmp_fn, 'size'=>$full_size, 'status'=>-1,'info'=>'File name is not allowed. Windows reserved.');
		return false;
	}
	...
	{
		$response = array('name'=>$file_name, 'size'=>$full_size, 'status'=>-1,'info'=>'File extension is not allowed');
		return false;
	}
}
...
if($is_ajax)//Ajax Upload, FormData Upload and FF3.6 php://input upload
{
    ...
    if($isLast!='true')
    {
    	$response =array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>1, 'info'=>'Chunk uploaded');
    }
}
else //Normal html and flash upload
{
	...
	{
		$response = array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>-1, 'info'=>'File move error');
		return  false;
	}
}
...
if($isLast == 'true')
{
	...
	$response = array('name'=>basename($full_path), 'size'=>$full_size, 'status'=>1, 'info'=>'File uploaded');
	@success($full_path);
}